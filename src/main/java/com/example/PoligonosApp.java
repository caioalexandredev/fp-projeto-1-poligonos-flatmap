...
public class PoligonosApp extends Application {
    ...
    public void start(final Stage mainStage) {
        ...
        final var label1 = newLabel("Perímetro dos Polígonos: " +
                perimetros().stream().map(p -> String.format("%.1f", p)).toList(), 500);
    ...
    protected List<Double> perimetros(){
        return pontosPoligonos.stream()
                .map(poligono -> {
                    Point pontoInicial = new Point(poligono.get(poligono.size() - 1), poligono.get(0));
                    return poligono.stream()
